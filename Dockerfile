...
RUN \
  ...
  echo "**** setup nginx ****" && \
  cp -f \
    /tmp/immich/nginx/nginx.conf \
    /etc/nginx/nginx.conf && \
  sed -i \
    "s+\$IMMICH_SERVER_URL+$IMMICH_SERVER_URL+g; \
    s+\$IMMICH_WEB_URL+$IMMICH_WEB_URL+g; \
    s+\${IMMICH_SERVER_SCHEME}+$IMMICH_SERVER_SCHEME+g; \
    s+\${IMMICH_WEB_SCHEME}+$IMMICH_WEB_SCHEME+g; \
    s+\$IMMICH_SERVER_HOST+$IMMICH_SERVER_HOST+g; \
    s+\$IMMICH_WEB_HOST+$IMMICH_WEB_HOST+g; \
    s+/tmp/nginx.pid+/run/immich/nginx.pid+g; \
    s+/var/log/nginx/error.log+/config/log/nginx/error.log+g" \
    /etc/nginx/nginx.conf && \
  echo -e "daemon off;\nuser abc;" >> /etc/nginx/nginx.conf && \ 
  echo "**** cleanup ****" && \
  chown -R abc:abc /app && \
