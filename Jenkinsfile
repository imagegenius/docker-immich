  ...
  stages {
    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.IG_RELEASE = sh(
            script: '''docker run --rm ghcr.io/linuxserver/alexeiled-skopeo sh -c 'skopeo inspect docker://ghcr.io/'${IG_USER}'/'${CONTAINER_NAME}':dev 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ig' || : ''',
        ...
        script{
          env.IG_TAG_NUMBER = sh(
            script: '''#!/bin/bash
                       tagsha=$(git rev-list -n 1 dev-${IG_RELEASE} 2>/dev/null)
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-dev-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER + '|arm64v8-dev-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          } else {
            env.CI_TAGS = 'dev-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          env.META_TAG = 'dev-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
    ...
    stage("Set ENV PR build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-dev-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm64v8-dev-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          } else {
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#!/bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=dev -v ${TEMPDIR}:/ansible/jenkins ghcr.io/imagegenius/jenkins-builder:latest 
      ...
      steps {
        sh '''#!/bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${IG_REPO}/.git checkout -f dev
    ...
    stage('Docker-Push-Single') {
      ...
      steps {
        sh '''#!/bin/bash
              set -e
              echo $GITHUB_TOKEN | docker login ghcr.io -u ImageGeniusCI --password-stdin
              docker tag ${GITHUBIMAGE}:${META_TAG} ${GITHUBIMAGE}:dev
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        sh '''#!/bin/bash
              ...
              docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:amd64-dev
              ...
              docker push ${GITHUBIMAGE}:amd64-dev
              ...
              docker manifest push --purge ${GITHUBIMAGE}:dev || :
              docker manifest create ${GITHUBIMAGE}:dev ${GITHUBIMAGE}:amd64-dev ${GITHUBIMAGE}:arm64v8-dev
              docker manifest annotate ${GITHUBIMAGE}:dev ${GITHUBIMAGE}:arm64v8-dev --os linux --arch arm64 --variant v8
        ...
        sh '''#!/bin/bash
              docker rmi \
                ...
                ${GITHUBIMAGE}:amd64-dev \
                ${GITHUBIMAGE}:amd64-${EXT_RELEASE_TAG} \
                ${GITHUBIMAGE}:arm64v8-${META_TAG} \
                ${GITHUBIMAGE}:arm64v8-dev \
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${META_TAG}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ig'${IG_TAG_NUMBER}' to dev",\
             "type": "commit",\
      ...
      script{
        if (env.EXIT_STATUS == "ABORTED"){
          ...
          sh ''' curl -X POST -H "Content-Type: application/json" --data '{"avatar_url": "https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/jenkins-avatar.png","embeds": [{"color": 1681177,\
                 "description": "**'${IG_REPO}' Build '${BUILD_NUMBER}' (dev)**\\n**CI Results:**  '${CI_URL}'\\n**Job:** '${RUN_DISPLAY_URL}'\\n**Changes:** '${CODE_URL}'\\n**External Release:** '${RELEASE_LINK}'\\n"}],\
                 "username": "Jenkins"}' ${BUILDS_DISCORD} '''
        }
        else {
          sh ''' curl -X POST -H "Content-Type: application/json" --data '{"avatar_url": "https://raw.githubusercontent.com/linuxserver/docker-templates/master/linuxserver.io/img/jenkins-avatar.png","embeds": [{"color": 16711680,\
                 "description": "**'${IG_REPO}' Build '${BUILD_NUMBER}' Failed! (dev)**\\n**CI Results:**  '${CI_URL}'\\n**Job:** '${RUN_DISPLAY_URL}'\\n**Change:** '${CODE_URL}'\\n**External Release:** '${RELEASE_LINK}'\\n"}],\
                 "username": "Jenkins"}' ${BUILDS_DISCORD} '''
        }
      }
    }
