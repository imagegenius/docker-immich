    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              # ${TEMPDIR}/docker-${CONTAINER_NAME}: Cloned branch alpine of ${IG_USER}/${IG_REPO} for running the jenkins builder on
              # ${TEMPDIR}/repo/${IG_REPO}: Cloned branch alpine of ${IG_USER}/${IG_REPO} for commiting various templated file changes and pushing back to Github
              ...
              if [[ "$(md5sum Jenkinsfile | awk '{ print $1 }')" != "$(md5sum ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile | awk '{ print $1 }')" ]]; then
                ...
                git pull https://ImageGeniusCI:${GITHUB_TOKEN}@github.com/${IG_USER}/${IG_REPO}.git alpine
                git push https://ImageGeniusCI:${GITHUB_TOKEN}@github.com/${IG_USER}/${IG_REPO}.git alpine
      ...
      steps {
        retry(5) {
          sh '''#! /bin/bash
                ...
                docker buildx imagetools create -t ${GITHUBIMAGE}:alpine ${GITHUBIMAGE}:amd64-alpine ${GITHUBIMAGE}:arm64v8-alpine
