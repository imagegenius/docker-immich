    ...
    stage("Set ENV Variables base"){
      steps{
        ...
        script{
          env.EXIT_STATUS = ''
          env.IG_RELEASE = sh(
            script: '''docker run --rm quay.io/skopeo/stable:v1 inspect docker://ghcr.io/${IG_USER}/${CONTAINER_NAME}:openvino 2>/dev/null | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ig' || : ''',
        ...
        script{
          env.IG_TAG_NUMBER = sh(
            script: '''#! /bin/bash
                       tagsha=$(git rev-list -n 1 openvino-${IG_RELEASE} 2>/dev/null)
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-openvino-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER + '|arm64v8-openvino-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          } else {
            env.CI_TAGS = 'openvino-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          env.META_TAG = 'openvino-' + env.EXT_RELEASE_CLEAN + '-ig' + env.IG_TAG_NUMBER
          env.EXT_RELEASE_TAG = 'openvino-version-' + env.EXT_RELEASE_CLEAN
        }
      }
    }
    ...
    stage("Set ENV dev build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA + '|arm64v8-openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          } else {
            env.CI_TAGS = 'openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          env.META_TAG = 'openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          env.EXT_RELEASE_TAG = 'openvino-version-' + env.EXT_RELEASE_CLEAN
        }
      }
    }
    ...
    stage("Set ENV PR build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm64v8-openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          } else {
            env.CI_TAGS = 'openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          env.META_TAG = 'openvino-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          env.EXT_RELEASE_TAG = 'openvino-version-' + env.EXT_RELEASE_CLEAN
      ...
      steps {
        retry(5) {
          sh '''#! /bin/bash
                ...
                docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:amd64-openvino
                ...
                docker tag ${GITHUBIMAGE}:arm64v8-${META_TAG} ${GITHUBIMAGE}:arm64v8-openvino
                ...
                docker push ${GITHUBIMAGE}:amd64-openvino
                docker push ${GITHUBIMAGE}:arm64v8-${META_TAG}
                docker push ${GITHUBIMAGE}:arm64v8-openvino
                ...
                docker manifest push --purge ${GITHUBIMAGE}:openvino || :
                docker manifest create ${GITHUBIMAGE}:openvino ${GITHUBIMAGE}:amd64-openvino ${GITHUBIMAGE}:arm64v8-openvino
                docker manifest annotate ${GITHUBIMAGE}:openvino ${GITHUBIMAGE}:arm64v8-openvino --os linux --arch arm64 --variant v8
                ...
                digest=$(curl -s \
                  ...
                  "https://ghcr.io/v2/imagegenius/${CONTAINER_NAME}/manifests/arm32v7-openvino")
                if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                  docker manifest push --purge ${GITHUBIMAGE}:arm32v7-openvino || :
                  docker manifest create ${GITHUBIMAGE}:arm32v7-openvino ${GITHUBIMAGE}:amd64-openvino
                  docker manifest push --purge ${GITHUBIMAGE}:arm32v7-openvino
                fi
                docker manifest push --purge ${GITHUBIMAGE}:openvino
                docker manifest push --purge ${GITHUBIMAGE}:${META_TAG} 
